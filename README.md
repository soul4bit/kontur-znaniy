...
- `editor` — просмотр + создание/редактирование статей.
- `admin` — все права + управление пользователями в `/app/admin/users`.

По умолчанию новые пользователи после подтверждения email получают роль `viewer`.

### Как выдать первого админа

Если в системе еще нет администратора, выдайте роль напрямую в БД:
